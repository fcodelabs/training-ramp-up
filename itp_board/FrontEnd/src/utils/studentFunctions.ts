...
export const getStoreData = () => {
  const editId = useAppSelector((state) => {
  ...
  const newAdded = useAppSelector((state) => {
  ...
  const editingField = useAppSelector((state) => {
...
export const validate = (data: Student | null) => {
  ...
  if (
    data !== null &&
    !isValidDateOfBirth(data.dateOfBirth ? new Date(data.dateOfBirth) : null).state
  ) {
